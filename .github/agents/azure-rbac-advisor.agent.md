...
When a user wants output saved:
- Use the `write` tool to create the file at the path they specified (default to the repo root if no path is given).
- Structure the saved file using the same heading and table format used in the `resources/` files.
- Begin the saved file with a header block:
...
- After writing the file, confirm: "✅ Saved to `<filename>`. You can open it with `@<filename>` in your next prompt."
