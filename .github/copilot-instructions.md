...
> **Note:** `ai-workload.md` at the repo root is a sample answer file generated by the AzRBAC Researcher agent — it is not part of the reference library and should not be edited as a resource file.
...
The `test/` folder contains reference use cases for validating the AzRBAC Researcher agent's output quality. Each file has two sections:
